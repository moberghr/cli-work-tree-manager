...
- **Live server (`wd`, default):** `runLauncher` calls `ensureWorkWebRunning` (reads `web.url`, autostarts lean work web on a 5 s wait if absent), POSTs the scope, opens `/diff/<hash>`. There is no per-scope daemon class anymore — the previous `runDaemon` / `spawnDaemon` / per-scope `.pid|.url|.log` files are gone. Live updates via chokidar + SSE are **staged, never auto-applied**: on `diff-changed` the SPA re-fetches in the background (`checkForUpdates` in `hooks/use-deferred-diff-load.ts`) and, only if the payload actually differs, parks it in `pending` and shows a `DiffUpdateChip` ("New changes · N files" + Show / Reload — Show applies the staged payload keeping scroll, Reload is a soft foreground refetch + scroll-to-top, never `location.reload()`) **in the header** — the diff itself never moves under a reader mid-scroll, and nothing above it is inserted (a banner would shift every line). § WHEN adding another live-refresh path, stage it the same way; do not write straight into `data`. `wd --stop` de-registers the scope (DELETE `/api/scopes/:hash`); the server itself keeps running.
